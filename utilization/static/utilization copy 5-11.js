...
if ((list_length/5)*6.5+4>height){
    draw_histogram(data, bins, margin, height, width, list_length, bin_values, g, x);
} else{
...
function draw_dot_chart(bins, margin, height, width, list_length, bin_values, g, x, x_visual){
    ...
    var tip = d3.tip()
        ...
        return "<strong>"+d.test+"</strong> <span style='color:rgba(212,43,0, 1)'>" + d.value + "</span><br> \
                <strong>Provider:</strong> <span style='color:rgba(212,43,0, 1)'>" + d.provider + "</span>";
    })
...
function draw_histogram(data, bins, margin, height, width, list_length, bin_values, g, x){
